...
Public Class TrayApp
    ...
    Private Function GetAvailablePorts() As List(Of String)
        ...
        ' Использование WMI для получения списка COM-портов
        ...
        For Each device As ManagementObject In searcher.Get()
            ...
            Dim match As System.Text.RegularExpressions.Match = System.Text.RegularExpressions.Regex.Match(caption, "\(COM\d+\)")
            If match.Success Then
                Dim port As String = match.Value.Replace("(", "").Replace(")", "")
                Try
                ...
                Catch ex As UnauthorizedAccessException
                    ' Порт занят
                    portList.Add(port & " - " & GetLocalizedString("Busy"))
                Catch ex As Exception
                    ' Порт недоступен, пропускаем его
                End Try
            End If
        Next
